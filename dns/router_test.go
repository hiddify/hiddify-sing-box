...
type fakeDNSSimpleTransport struct {
	tag           string
	transportType string
}

func (t *fakeDNSSimpleTransport) Start(adapter.StartStage) error { return nil }
func (t *fakeDNSSimpleTransport) Close() error                   { return nil }
func (t *fakeDNSSimpleTransport) Type() string                   { return t.transportType }
func (t *fakeDNSSimpleTransport) Tag() string                    { return t.tag }
func (t *fakeDNSSimpleTransport) Dependencies() []string         { return nil }
func (t *fakeDNSSimpleTransport) Reset()                         {}
func (t *fakeDNSSimpleTransport) Exchange(context.Context, *mDNS.Msg) (*mDNS.Msg, error) {
	return nil, E.New("unused transport exchange")
}

func (t *fakeDNSSimpleTransport) ExchangeAsync(ctx context.Context, message *mDNS.Msg, callback func(response *mDNS.Msg, err error)) {
	callback(t.Exchange(ctx, message))
}

type fakeDNSSimpleTransportManager struct {
	defaultTransport adapter.DNSTransport
	transports       map[string]adapter.DNSTransport
}

func (m *fakeDNSSimpleTransportManager) Start(adapter.StartStage) error { return nil }
func (m *fakeDNSSimpleTransportManager) Close() error                   { return nil }
func (m *fakeDNSSimpleTransportManager) Transports() []adapter.DNSTransport {
...
func (m *fakeDNSSimpleTransportManager) Transport(tag string) (adapter.DNSTransport, bool) {
	transport, loaded := m.transports[tag]
	return transport, loaded
}
func (m *fakeDNSSimpleTransportManager) Default() adapter.DNSTransport { return m.defaultTransport }
func (m *fakeDNSSimpleTransportManager) FakeIP() adapter.FakeIPTransport {
	return nil
}
func (m *fakeDNSSimpleTransportManager) Remove(string) error { return nil }
func (m *fakeDNSSimpleTransportManager) Create(context.Context, log.ContextLogger, string, string, any) error {
...
func (r *fakeRouter) PreMatch(adapter.InboundContext, []byte) adapter.PreMatchResult {
	return adapter.PreMatchResult{}
}

func (r *fakeRouter) HijackDNSPacket(context.Context, []byte, N.PacketWriter, adapter.InboundContext) {
}
...
func newTestRouter(t *testing.T, rules []option.DNSRule, transportManager *fakeDNSSimpleTransportManager, client *fakeDNSClient) *Router {
...
func newTestRouterWithContext(t *testing.T, ctx context.Context, rules []option.DNSRule, transportManager *fakeDNSSimpleTransportManager, client *fakeDNSClient) *Router {
	return newTestRouterWithContextAndLogger(t, ctx, rules, transportManager, client, log.NewNOPFactory().NewLogger("dns"))
}

func newTestRouterWithContextAndLogger(t *testing.T, ctx context.Context, rules []option.DNSRule, transportManager *fakeDNSSimpleTransportManager, client *fakeDNSClient, dnsLogger log.ContextLogger) *Router {
	t.Helper()
	router := &Router{
		ctx:                   ctx,
		logger:                dnsLogger,
...
func TestInitializeRejectsDirectLegacyRuleWhenRuleSetForcesNew(t *testing.T) {
	...
	router := &Router{
		...
		transport:             &fakeDNSSimpleTransportManager{},
...
func TestLookupLegacyDNSModeDefersRuleSetDestinationIPMatch(t *testing.T) {
	...
	defaultTransport := &fakeDNSSimpleTransport{tag: "default", transportType: C.DNSTypeUDP}
	privateTransport := &fakeDNSSimpleTransport{tag: "private", transportType: C.DNSTypeUDP}
...
func TestValidateRuleSetMetadataUpdateRejectsRuleSetThatWouldDisableLegacyDNSMode(t *testing.T) {
	...
	router := newTestRouterWithContext(t, ctx, []option.DNSRule{
		...
		defaultTransport: &fakeDNSSimpleTransport{tag: "default", transportType: C.DNSTypeUDP},
		transports: map[string]adapter.DNSTransport{
			"default":  &fakeDNSSimpleTransport{tag: "default", transportType: C.DNSTypeUDP},
			"selected": &fakeDNSSimpleTransport{tag: "selected", transportType: C.DNSTypeUDP},
		},
	}, &fakeDNSClient{
...
func TestValidateRuleSetMetadataUpdateAllowsRuleSetThatKeepsNonLegacyDNSMode(t *testing.T) {
	...
	router := newTestRouterWithContext(t, ctx, []option.DNSRule{
		...
		transports: map[string]adapter.DNSTransport{
			"default":  &fakeDNSSimpleTransport{tag: "default", transportType: C.DNSTypeUDP},
			"selected": &fakeDNSSimpleTransport{tag: "selected", transportType: C.DNSTypeUDP},
			"upstream": &fakeDNSSimpleTransport{tag: "upstream", transportType: C.DNSTypeUDP},
		},
	}, &fakeDNSClient{})
	require.False(t, router.legacyDNSMode)
...
func TestCloseWaitsForInFlightLookupUntilContextCancellation(t *testing.T) {
	...
	defaultTransport := &fakeDNSSimpleTransport{tag: "default", transportType: C.DNSTypeUDP}
	selectedTransport := &fakeDNSSimpleTransport{tag: "selected", transportType: C.DNSTypeUDP}
...
func TestExchangeLegacyDNSModeDisabledEvaluateMatchResponseRoute(t *testing.T) {
	...
	transportManager := &fakeDNSSimpleTransportManager{
		defaultTransport: &fakeDNSSimpleTransport{tag: "default", transportType: C.DNSTypeUDP},
		transports: map[string]adapter.DNSTransport{
			"upstream": &fakeDNSSimpleTransport{tag: "upstream", transportType: C.DNSTypeUDP},
			"selected": &fakeDNSSimpleTransport{tag: "selected", transportType: C.DNSTypeUDP},
...
func TestExchangeLegacyDNSModeDisabledSecondEvaluateOverwritesFirstResponse(t *testing.T) {
	...
	transportManager := &fakeDNSSimpleTransportManager{
		...
		transports: map[string]adapter.DNSTransport{
			"default":         &fakeDNSSimpleTransport{tag: "default", transportType: C.DNSTypeUDP},
			"first-upstream":  &fakeDNSSimpleTransport{tag: "first-upstream", transportType: C.DNSTypeUDP},
			"second-upstream": &fakeDNSSimpleTransport{tag: "second-upstream", transportType: C.DNSTypeUDP},
			"first-match":     &fakeDNSSimpleTransport{tag: "first-match", transportType: C.DNSTypeUDP},
			"second-match":    &fakeDNSSimpleTransport{tag: "second-match", transportType: C.DNSTypeUDP},
		},
	}
	client := &fakeDNSClient{
		exchange: func(transport adapter.DNSTransport, message *mDNS.Msg) (*mDNS.Msg, error) {
...
func TestLookupLegacyDNSModeDisabledAllowsPartialSuccessForExchangeFailure(t *testing.T) {
	...
	defaultTransport := &fakeDNSSimpleTransport{tag: "default", transportType: C.DNSTypeUDP}
	router := newTestRouter(t, nil, &fakeDNSSimpleTransportManager{
		defaultTransport: defaultTransport,
		transports: map[string]adapter.DNSTransport{
			"default": defaultTransport,
		},
...
func TestLookupLegacyDNSModeDisabledSkipsFakeIPRule(t *testing.T) {
	...
	router := newTestRouter(t, []option.DNSRule{{
		...
		transports: map[string]adapter.DNSTransport{
			"default": defaultTransport,
			"fake":    &fakeDNSSimpleTransport{tag: "fake", transportType: C.DNSTypeFakeIP},
		},
	}, &fakeDNSClient{
		exchange: func(transport adapter.DNSTransport, message *mDNS.Msg) (*mDNS.Msg, error) {
			require.Equal(t, "default", transport.Tag())
...
func TestExchangeLegacyDNSModeDisabledAllowsRouteFakeIPRule(t *testing.T) {
	...
	fakeTransport := &fakeDNSSimpleTransport{tag: "fake", transportType: C.DNSTypeFakeIP}
	router := newTestRouter(t, []option.DNSRule{{
		Type: C.RuleTypeDefault,
		DefaultOptions: option.DefaultDNSRule{
			RawDefaultDNSRule: option.RawDefaultDNSRule{
...
func TestInitializeRejectsEvaluateFakeIPServerInDefaultRule(t *testing.T) {
	...
	router := &Router{
		ctx:                   context.Background(),
		logger:                log.NewNOPFactory().NewLogger("dns"),
		transport:             &fakeDNSSimpleTransportManager{transports: map[string]adapter.DNSTransport{"fake": &fakeDNSSimpleTransport{tag: "fake", transportType: C.DNSTypeFakeIP}}},
		client:                &fakeDNSClient{},
		rawRules:              make([]option.DNSRule, 0, 1),
		defaultDomainStrategy: C.DomainStrategyAsIS,
	}
...
func TestExchangeLegacyDNSModeDisabledReturnsDropErrorForRejectDropAction(t *testing.T) {
	...
	require.Nil(t, response)
	require.ErrorIs(t, err, rulepkg.ErrDrop)
}
