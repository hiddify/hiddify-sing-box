...
func NewClient(ctx context.Context, dialer N.Dialer, serverAddr M.Socksaddr, options option.V2RayXHTTPOptions, tlsConfig tls.Config) (adapter.V2RayClientTransport, error) {
	...
	if dest.IsFqdn() {
		addresses, err := dnsRouter.Lookup(ctx, dest.Fqdn, adapter.DNSQueryOptions{Strategy: C.DomainStrategy(options.DomainStrategy)})
		if err != nil {
			return nil, err
		}
	...
	if options.Download != nil {
		...
		if dest2.IsFqdn() {
			addresses2, err := dnsRouter.Lookup(ctx, dest2.Fqdn, adapter.DNSQueryOptions{Strategy: C.DomainStrategy(options2.DomainStrategy)})
			if err != nil {
				return nil, err
			}
